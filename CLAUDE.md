...
这不是一个应用代码仓库，而是 **gfeu-code-copilot 框架本身的源码**——一套面向后端项目的 Claude Code 编码协作框架（Spec 驱动 + 渐进式复杂度）。仓库内容是 Markdown 提示词、Sub-Agent 定义、规则文件和安装脚本，没有需要编译的业务代码。

编辑本仓库时，你修改的是**框架的行为**；它通过 `install.sh` 安装到 `~/.claude/` 后，会作为 skill 注入到其他项目的 Claude Code 会话里运行。

## 安装与分发机制（理解这点才能正确改框架）

`install.sh` / `install.ps1` 做三件事，改动相关文件时要保持一致：

1. **clone/pull 到** `~/.claude/gfeu-code-copilot/`（本仓库的部署位置）
2. **symlink** `~/.claude/skills/gfeu-code-copilot` → `<install>/skill/`（注册 skill；Windows 用 Junction）
3. **注入 SessionStart hook** 到 `~/.claude/settings.json`，每次会话运行 `hooks/session-start`

关键路径耦合：提示词里大量硬编码 `~/.claude/gfeu-code-copilot/...` 绝对路径（如 `agents/copilot-prompt.md`、`packs/`、`rules/`）。**移动或重命名顶层目录会破坏这些引用**——改目录结构时必须同步 grep 全仓库的路径引用。
...
| `skill/SKILL.md` | Claude Code skill 注册入口 | description 决定何时触发；正文指向 copilot-prompt |
...
- 全局：`~/.claude/gfeu-code-copilot/rules|knowledge|...`（即本仓库部署后的位置）
...
- **部门级**：`~/.claude/gfeu-knowledge/` — 跨项目复用的技术约定/踩坑/反模式，是一个**独立 git 仓库**，部门成员各自 clone 到此固定路径
- 全局模板：`~/.claude/gfeu-code-copilot/knowledge/` — 仅模板
...
bash install.sh                 # 在隔离环境验证安装/更新链路（会写 ~/.claude/）
